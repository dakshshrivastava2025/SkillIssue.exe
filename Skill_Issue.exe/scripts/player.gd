...
func _perform_attack() -> void:
	...
	if attack_area:
		for body in attack_area.get_overlapping_bodies():
			if body != self and body not in hit_targets and (body.is_in_group("enemy") or body.is_in_group("boss") or body.is_in_group("statue")):
				hit_targets.append(body)
		for area in attack_area.get_overlapping_areas():
			var parent = area.get_parent()
			if parent and parent != self and parent not in hit_targets and (parent.is_in_group("enemy") or parent.is_in_group("boss") or parent.is_in_group("statue")):
				hit_targets.append(parent)

	# Also scan enemies (and dormant statues) within attack_range in front of player
	var aim_dir = facing_vector.normalized()
	var scan_groups = get_tree().get_nodes_in_group("enemy") + get_tree().get_nodes_in_group("boss") + get_tree().get_nodes_in_group("statue")
	for enemy in scan_groups:
