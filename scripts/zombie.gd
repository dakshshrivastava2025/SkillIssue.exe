...
func _perform_melee_attack(to_target: Vector2) -> void:
	...
	if _target and is_instance_valid(_target) and _target.has_method("take_damage"):
		var live_dist = global_position.distance_to(_target.global_position)
		# Fair dodge check: player only takes damage if they haven't dashed or moved out of attack range
		if live_dist <= melee_range * 1.35:
